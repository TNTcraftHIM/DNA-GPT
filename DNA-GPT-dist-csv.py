...
def generate_html(key, option, text ):
    ...
    # calculate the AUROC
    auroc_list = process_p_values_and_labels(answer_labels, results)
    all_auroc = process_p_values_and_labels_odd(answer_labels, results)
    ...
    html_text += "<p>avg_auroc: {:.4f}".format(sum(auroc_list) / len(auroc_list)) + "; ".join(
        ["std_auroc: {:.4f}".format(np.std(auroc_list))]) + "</p>"
    html_text += "<p>all_auroc: " + str(all_auroc) + "</p>"
    html_text += "<p>auroc_list: " + str(auroc_list) + "</p>"
    return html_text
