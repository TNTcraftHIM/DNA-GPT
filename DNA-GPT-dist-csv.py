...
def detection(text, option):
    ...
    for i in range(regen_number):  # len(human_half)
        temp1 = {}
        if option == "GPT-3.5":
            gen_text = completion.choices[i].text
        else:
    ...
    for instance in ngram_overlap_count:
        ...
        most_matched_generatedtext.append(
            completion.choices[max_index].text if option == "GPT-3.5" else completion.choices[max_index].message.content)
    print(gpt_scores[0])
    ...
    if gpt_scores[0] > threshold:
        ...
        decision = False
    return decision, most_matched_generatedtext[0], gpt_scores[0]
...
def generate_html(key, option,):
    ...
    for text in csv['text']:
        res, max_over_lap_generatedtext, score = detection(text, option)
        results.append(score)
    # calculate the AUROC
    all_auroc, all_avg_fpr, all_std_fpr, all_avg_tpr, all_std_tpr = process_p_values_and_labels_odd(
        answer_labels, results)

    html_text = "<h3>" + label + "</h3>"
    html_text += "<p> The AUROC of the DNA-GPT detection tool is: " + str(all_auroc) + "</p>"
    html_text += "<p> The average FPR of the DNA-GPT detection tool is: " + str(all_avg_fpr) + "</p>"
    html_text += "<p> The standard deviation of FPR of the DNA-GPT detection tool is: " + str(all_std_fpr) + "</p>"
    html_text += "<p> The average TPR of the DNA-GPT detection tool is: " + str(all_avg_tpr) + "</p>"
    html_text += "<p> The standard deviation of TPR of the DNA-GPT detection tool is: " + str(all_std_tpr) + "</p>"
    return html_text
...
def main():
    ...
    input_group = [input_key, radio]

    greet_output = gr.outputs.Textbox(label="Detection Output:")

    interface = gr.Interface(fn=generate_html, inputs=input_group, title="DNA-GPT Demo (NEC Labs America) - Local CSV Detection",
